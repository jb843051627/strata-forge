...
func (s *LabService) ReviewMeasurement(ctx context.Context, id int64, in model.ReviewInput) (model.Review, error) {
	...
	measurement, err := s.GetMeasurement(context.Background(), id)
	if err != nil {
		return model.Review{}, err
	}
	...
	layer, err := s.GetLayer(context.Background(), measurement.LayerID)
	if err != nil {
		return model.Review{}, err
	}
	now := s.clock.Now()
	review := model.Review{MeasurementID: id, Decision: in.Decision, Comment: in.Comment, Reviewer: in.Reviewer, CreatedAt: now}
	status := s.workflow.ReviewStatus(in.Decision)
	if err := s.store.SaveMeasurementReview(context.Background(), measurement, review, layer.SampleID, status); err != nil {
