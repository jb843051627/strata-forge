...
func (s *Store) CreateSample(ctx context.Context, item model.Sample) (model.Sample, error) {
	result, err := s.execContext(ctx, `INSERT INTO samples(code, site, depth_start, depth_end, status, received_at, notes) VALUES(?, ?, ?, ?, ?, ?, ?)`, item.Code, item.Site, item.DepthStart, item.DepthEnd, item.Status, clock.Format(item.ReceivedAt), item.Notes)
	if err != nil {
...
func (s *Store) FindSampleByCode(ctx context.Context, code string) (model.Sample, error) {
	...
	item, err := scanSample(row)
	if errors.Is(err, sql.ErrNoRows) {
		return model.Sample{}, fmt.Errorf("find sample: %v", model.ErrNotFound)
	}
	if err != nil {
