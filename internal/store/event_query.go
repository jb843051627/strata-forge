...
func (s *Store) EventsSince(ctx context.Context, sampleID int64, since time.Time) ([]model.Event, error) {
	rows, err := s.queryContext(context.Background(), `SELECT `+eventColumns+` FROM events WHERE sample_id = ? AND created_at >= ? ORDER BY created_at, id`, sampleID, clock.Format(since))
	if err != nil {
