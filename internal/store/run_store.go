...
func (s *Store) GetRun(ctx context.Context, id int64) (model.Run, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return model.Run{}, fmt.Errorf("get run: %v", model.ErrNotFound)
	}
	if err != nil {
...
func (s *Store) FindActiveRun(ctx context.Context, sampleID int64) (model.Run, error) {
	...
	item, err := scanRun(row)
	if errors.Is(err, sql.ErrNoRows) {
		return model.Run{}, fmt.Errorf("find active run: %v", model.ErrNotFound)
	}
	if err != nil {
...
func (s *Store) UpdateRunAndSample(ctx context.Context, run model.Run, sampleID int64, sampleStatus string) error {
	return s.withTx(ctx, func(tx *sql.Tx) error {
		...
		if run.StartedAt != nil {
			started = clock.Format(*run.StartedAt)
		}
		if run.FinishedAt != nil {
			finished = clock.Format(*run.FinishedAt)
		}
		if _, err := tx.ExecContext(ctx, `UPDATE runs SET state = ?, started_at = ?, finished_at = ?, cancel_note = ? WHERE id = ?`, run.State, started, finished, run.CancelNote, run.ID); err != nil {
			return fmt.Errorf("update run in transaction: %w", err)
		}
		result, err := tx.ExecContext(ctx, `UPDATE samples SET status = ? WHERE id = ?`, sampleStatus, sampleID)
		if err != nil {
