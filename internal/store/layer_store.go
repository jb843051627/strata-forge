...
func (s *Store) CreateLayer(ctx context.Context, item model.Layer) (model.Layer, error) {
	result, err := s.execContext(context.Background(), `INSERT INTO layers(sample_id, sequence_no, top_depth, bottom_depth, material, status, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)`, item.SampleID, item.Sequence, item.TopDepth, item.BottomDepth, item.Material, item.Status, clock.Format(item.CreatedAt))
	if err != nil {
...
func (s *Store) GetLayer(ctx context.Context, id int64) (model.Layer, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return model.Layer{}, model.ErrNotFound
	}
	if err != nil {
